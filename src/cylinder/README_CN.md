# MAVROS Offboard 圆柱螺旋轨迹控制

本 ROS 节点通过 MAVROS 功能包实现 PX4 飞控的自主圆柱螺旋航线飞行。无人机以恒定半径围绕垂直轴螺旋上升。

[ENGLISH](README.md)

## 功能描述

* **轨迹生成**：使用 NumPy 预计算圆柱螺旋路径，可配置半径、高度范围和螺旋圈数。
...
在进入控制循环前，利用极坐标公式将圆柱螺旋离散化为 $(x, y, z)$ 坐标数组:
...
其中:
- $R=2.0m$ (圆柱半径)
- $\theta$ 范围从 $0$ 到 $2\pi \times n_{spirals}$
- 高度从 $z_{start}=1.5m$ 线性插值到 $z_{end}=4.0m$
