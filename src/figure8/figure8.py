...
# 生成8字形轨迹点 (Lissajous曲线)
# x = A * sin(t)
# y = B * sin(2t) / 2  (频率为2，形成8字)
t = np.linspace(0, 2 * np.pi, num_points)
x_coords = AMPLITUDE_X * np.sin(t) + xc
y_coords = AMPLITUDE_Y * np.sin(2 * t) / 2 + yc
z_coords = np.full(num_points, HEIGHT)
...
if __name__ == "__main__":
    rospy.init_node("figure8_trajectory_node")
    RATE_HZ = 20.0
