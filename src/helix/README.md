# MAVROS Offboard Helix Trajectory Control

This ROS node implements an autonomous helical flight path for PX4-based drones using the MAVROS package. The drone ascends in a spiral pattern with adjustable radius, allowing for both cylindrical and conical spiral configurations.

[简体中文](README_CN.md)

## Functionality
* **Trajectory Generation:** Pre-calculates a helical spiral path with configurable start/end radius and height using NumPy.
...
* **Continuous Looping:** Once the helix completes, the trajectory index resets, creating an infinite flight loop.
* **Flexible Design:** When `R_START ≠ R_END`, creates a conical spiral; when equal, creates a cylindrical spiral.

## Implementation Logic

### 1. Trajectory Pre-calculation
The helix is discretized into an array of coordinates $(x, y, z)$ before the control loop starts:
$$x = r(t) \cdot \cos(\theta) + x_c$$
$$y = r(t) \cdot \sin(\theta) + y_c$$
$$z = z_{start} + \frac{(z_{end} - z_{start}) \cdot i}{n_{total}}$$

Where:
- $r(t)$ linearly interpolates from $R_{start}=2.0m$ to $R_{end}=2.0m$
- $\theta$ ranges from $0$ to $2\pi \times n_{turns}$
- Height linearly interpolates from $z_{start}=1.5m$ to $z_{end}=5.0m$
