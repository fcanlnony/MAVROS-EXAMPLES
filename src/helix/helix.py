...
# 轨迹参数
R_START = 2.0        # 起始半径 (m)
R_END = 2.0          # 结束半径 (m), 不等于start则为锥形螺旋
xc = 0.0             # 中心X
yc = 0.0             # 中心Y
...
radii = np.linspace(R_START, R_END, total_points)
heights = np.linspace(HEIGHT_START, HEIGHT_END, total_points)

x_coords = radii * np.cos(theta) + xc
y_coords = radii * np.sin(theta) + yc
z_coords = heights
...
if __name__ == "__main__":
    rospy.init_node("helix_trajectory_node")
    RATE_HZ = 20.0
